...
class HistAuGAN(pl.LightningModule):
    def __init__(self, opts, learning_rate=0.0001):
        ...
        self.enc_a = networks.EncoderAttribute(
            opts.input_dim,
            output_nc=self.dim_attribute,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % self.opts.log_train_img_freq == 0:
            self.log_images(
                image[:1], image[half_size : half_size + 1], domain[:1], domain[half_size : half_size + 1], "train"
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_l1_cc = torch.mean(torch.abs(image - torch.cat((fake_A_recon, fake_B_recon), 0))) * self.opts.lambda_rec
        ...
        if batch_idx % self.opts.log_val_img_freq == 0:
            self.log_images(
                image[:1],
                image[half_size : half_size + 1],
                domain[:1],
                domain[half_size : half_size + 1],
                "val",
                batch_idx,
            )
    ...
    def gen_loss(
        ...
        # -- self and cross-cycle recon
        loss_l1_self_recon = (
            torch.mean(torch.abs(image - torch.cat((fake_aa_encoded, fake_bb_encoded), 0))) * self.opts.lambda_rec
        )
        loss_l1_cc = torch.mean(torch.abs(image - torch.cat((fake_a_recon, fake_b_recon), 0))) * self.opts.lambda_rec
    ...
    def log_images(self, real_a, real_b, domain_a, domain_b, phase, batch_idx=0):
        ...
        mu_recon, log_var_recon = self.enc_a.forward(fake_encoded_img, domain)
        std_recon = log_var_recon.mul(0.5).exp_()
        eps_recon = torch.randn(std_recon.size(0), std_recon.size(1)).to(real_img.device)
        z_attr_recon = eps_recon.mul(std_recon).add_(mu_recon)
        ...
        img = torch.cat(
            (
                torch.cat(
                    (
                        real_a.detach().cpu(),
                        fake_A_encoded[:1].detach().cpu(),
                        fake_A_random[:1].detach().cpu(),
                        fake_AA_encoded[:1].detach().cpu(),
                        fake_A_recon[:1].detach().cpu(),
                    ),
                    dim=3,
                ),
                torch.cat(
                    (
                        real_b.detach().cpu(),
                        fake_B_encoded[:1].detach().cpu(),
                        fake_B_random[:1].detach().cpu(),
                        fake_BB_encoded[:1].detach().cpu(),
                        fake_B_recon[:1].detach().cpu(),
                    ),
                    dim=3,
                ),
            ),
    ...
    def configure_optimizers(self):
        dis_c_opt = torch.optim.Adam(
            self.dis_c.parameters(), lr=self.hparams.learning_rate / 2.5, betas=(0.5, 0.999), weight_decay=0.0001
        )
        dis1_opt = torch.optim.Adam(
            self.dis1.parameters(), lr=self.hparams.learning_rate, betas=(0.5, 0.999), weight_decay=0.0001
        )
        dis2_opt = torch.optim.Adam(
            self.dis2.parameters(), lr=self.hparams.learning_rate, betas=(0.5, 0.999), weight_decay=0.0001
        )
        enc_c_opt = torch.optim.Adam(
            self.enc_c.parameters(), lr=self.hparams.learning_rate, betas=(0.5, 0.999), weight_decay=0.0001
        )
        enc_a_opt = torch.optim.Adam(
            self.enc_a.parameters(), lr=self.hparams.learning_rate, betas=(0.5, 0.999), weight_decay=0.0001
        )
        gen_opt = torch.optim.Adam(
            self.gen.parameters(), lr=self.hparams.learning_rate, betas=(0.5, 0.999), weight_decay=0.0001
        )
        ...
        return [dis_c_opt, dis1_opt, dis2_opt, enc_c_opt, enc_a_opt, gen_opt], [
            dis_c_sch,
            dis1_sch,
            dis2_sch,
            enc_c_sch,
