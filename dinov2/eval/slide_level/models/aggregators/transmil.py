...
class TransMIL(BaseAggregator):
    ...
    def forward(self, x, coords=None):
        ...
        if self.pos_enc == "PPEG":
            ...
            add_length = (
                _H * _W - H
            )  # add N - n, first entries of feature vector added at the end to fill up until square number
            ...
            if h.shape[1] > 1:  # patient TCGA-A6-2675 has only one patch
                ...
                base_grid = torch.zeros(
                    (h.shape[0], dimensions_grid[0].int().item(), dimensions_grid[1].int().item(), h.shape[-1]),
                    device=h.device,
                )
